...
function getSumBetweenNumbers(n1, n2) {
  const numberOfTerms = n2 - n1 + 1;
  const sum = (numberOfTerms / 2) * (n1 + n2);
  return sum;
}
...
function findFirstSingleChar(str) {
  ...
  const nonRepeatedChar = charArray.find((char) => str.indexOf(char) === str.lastIndexOf(char));
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openingBrackets.includes(bracket)) {
      // Если это открывающая скобка, добавляем
      arr.push(bracket);
    } else if (closingBrackets.includes(bracket)) {
      // Если это закрывающая скобка, проверяем, соответствует ли она верхней скобке
      ...
      if (arr.length === 0 || arr.pop() !== matchingOpeningBracket) {
        // Если массив пуст или верхняя скобка в массиве не соответствует закрывающей скобке,
        // это ошибка
        return false;
      }
    }
  ...
  // Если в массиве остались открывающие скобки, это ошибка
  return arr.length === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPathes[0].length; i += 1) {
    ...
    for (let j = 1; j < splitPathes.length; j += 1) {
      if (splitPathes[j][i] !== currentSegment) {
